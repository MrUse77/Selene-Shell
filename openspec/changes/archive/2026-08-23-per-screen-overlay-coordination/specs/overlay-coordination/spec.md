...
Los siguientes escenarios SHALL ser verificables mediante tests deterministas de estado y, donde aplique, checks de runtime acotados con dos monitores:

| Categoría | Cobertura |
|-----------|-----------|
| Exclusividad | Tests de estado: apertura cierra previo, toggle, idempotencia |
| Captura de pantalla | Tests de estado: pantalla capturada no cambia con foco |
| Resolución widget | Tests de estado: apertura desde widget usa pantalla propietaria |
| Resolución IPC | Tests de estado: IPC usa monitor enfocado en instante de invocación |
| Hotplug | Tests de estado + runtime acotado: cierre por desconexión, idempotencia |
| Validación de origen | Tests de estado: origen en pantalla removida descartado, IPC/widget con origen vigente resuelto deterministamente |
| Fullscreen | Tests de estado: acción explícita vs pasiva sobre fullscreen |
| Invariantes | Tests de estado: secuencias rápidas, post-hotplug, pantalla capturada |
| Compatibilidad | Tests de regresión: nombres IPC, identidad visual, módulos fuera de alcance |
